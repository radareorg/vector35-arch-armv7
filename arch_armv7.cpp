...
enum PeArmRelocationType : uint32_t
{
	...
	PE_IMAGE_REL_ARM_BRANCH11 = 0x0004,  // The reference to a subroutine call. The reference consists
	...
	PE_IMAGE_REL_THUMB_BRANCH20 =
	    0x0012,  // The instruction is fixed up with the 21-bit relative displacement to the 2-byte
	             ...
	             // is not stored. This relocation corresponds to a Thumb-2 32-bit conditional B
	             // instruction.
	PE_IMAGE_REL_THUMB_UNUSED = 0x0013,  // Unused
	PE_IMAGE_REL_THUMB_BRANCH24 =
...
static BNRegisterInfo RegisterInfo(
...
class Armv7Architecture : public ArmCommonArchitecture
{
 protected:
	...
	virtual bool Disassemble(const uint8_t* data, uint64_t addr, size_t maxLen, Instruction& result)
	{
		...
		if (armv7_decompose(
		        *(uint32_t*)data, &result, (uint32_t)addr, (uint32_t)(m_endian == BigEndian)) != 0)
			return false;
		return true;
	}
	    ...
	    uint64_t addr, const Instruction& instr, InstructionInfo& result)
	{
		...
		case ARMV7_POP:
			...
			if (UNCONDITIONAL(instr.cond) && instr.operands[0].cls == REG_LIST &&
		...
		case ARMV7_LDMIB:
			...
			if (UNCONDITIONAL(instr.cond))
			{
				if (instr.operands[1].cls == REG_LIST &&
				    ((RegisterList)instr.operands[1].reg == REG_LIST_PC))
				{
					result.archTransitionByTargetAddr = true;
					result.AddBranch(UnresolvedBranch);
				}
				else if (instr.operands[1].cls == REG_LIST &&
				         ((instr.operands[1].reg & REG_LIST_PC) == REG_LIST_PC))
					result.AddBranch(FunctionReturn);
			}
			break;
		case ARMV7_ADC:
	...
	bool GetCoalescedLowLevelIL(
	    const uint8_t* data, uint64_t addr, size_t& len, LowLevelILFunction& il, Instruction& instr)
	{
		...
		for (size_t blockStart = 0; blockStart < disassembled;)
		{
			...
			auto liftInstructions = [&](Condition liftCond) {
				...
				// CASE 3: last instr was not a flag-setting instruction, goto next block of this cond (or
				// end)
				else
 ...
 public:
	Armv7Architecture(const char* arch, BNEndianness endian) : ArmCommonArchitecture(arch, endian) {}
	...
	static inline void GetSignedImmToken(
	    const InstructionOperand& op, vector<InstructionTextToken>& result)
	{
		char operand[32];
		const char* neg[2] = {"-", ""};
...
ArmCommonArchitecture::ArmCommonArchitecture(const char* name, BNEndianness endian) :
...
vector<uint32_t> ArmCommonArchitecture::GetFlagsRequiredForFlagCondition(
    BNLowLevelILFlagCondition cond, uint32_t)
{
	switch (cond)
	{
	case LLFC_E:
...
size_t ArmCommonArchitecture::GetFlagWriteLowLevelIL(BNLowLevelILOperation op, size_t size,
...
bool ArmCommonArchitecture::Assemble(
    const string& code, uint64_t addr, DataBuffer& result, string& errors)
{
	...
	errors.clear();
	assembleResult = BNLlvmServicesAssemble(code.c_str(), LLVM_SVCS_DIALECT_UNSPEC, triple.c_str(),
...
class ArmCallingConvention : public CallingConvention
{
 public:
	...
	virtual uint32_t GetHighIntegerReturnValueRegister() override { return REG_R1; }
};
	    ...
	    uint8_t* dest, size_t len) override
	{
		...
		case R_ARM_REL32:
			dest32[0] = swap((uint32_t)(
			    (target + (info.implicitAddend ? swap(dest32[0]) : info.addend)) - reloc->GetAddress()));
			break;
		case R_ARM_CALL:
		{
			if (target & 1)
			...
			_bl* bl = (_bl*)dest32;
			int64_t newTarget = (target + (info.implicitAddend ? ((bl->imm << 2) + 8) : info.addend)) -
			                    reloc->GetAddress();
			if ((newTarget - 8) > 0x3ffffff)
			{
				LogError("Unsupported relocation R_ARM_CALL @ 0x%" PRIx64
				         " with target greater than 0x3ffffff: 0x%" PRIx64,
				    reloc->GetAddress(), newTarget - 8);
				break;
			}
...
#pragma pack(pop)
			...
			int32_t curTarget =
			    (bl_hw2->offLo << 1) | (bl_hw1->offHi << 12) | (bl_hw1->sign ? (0xffc << 20) : 0);
			int32_t newTarget = (int32_t)(
			    (target + (info.implicitAddend ? curTarget : info.addend)) - reloc->GetAddress());
		...
		case R_ARM_PREL31:
		{
			...
			// if (sym)
			// {
			// 	LogError("%lx sym: %s dest32[0] %lx target: %lx", reloc->GetAddress(),
			// sym->GetFullName().c_str(), dest32[0], target);
			// }
			// else
			// {
			// 	LogError("%lx sym: null dest32[0] %lx target: %lx", reloc->GetAddress(), dest32[0],
			// target);
			// }
			dest32[0] = (info.implicitAddend ? dest32[0] : (uint32_t)info.addend) + (target & ~1) -
			            (uint32_t)reloc->GetAddress();
			break;
		}
		case R_ARM_JUMP24:
		{
			...
			_b* b = (_b*)dest32;
			int64_t newTarget = (target + (info.implicitAddend ? ((b->imm << 2) + 8) : info.addend)) -
			                    reloc->GetAddress();
			if ((newTarget - 8) > 0x3ffffff)
			{
				LogError("Unsupported relocation R_ARM_JUMP24 0x%" PRIx64
				         " with target greater than 0x3ffffff: 0x%" PRIx64,
				    reloc->GetAddress(), newTarget - 8);
				break;
			}
		...
		case R_ARM_MOVW_ABS_NC:
		{
			...
			int64_t newTarget =
			    (target + (info.implicitAddend ? (mov->imm4 << 12 | mov->imm12) : info.addend));
...
class ArmMachORelocationHandler : public RelocationHandler
{
 public:
	    ...
	    Ref<BinaryView> view, Ref<Architecture> arch, vector<BNRelocationInfo>& result) override
	{
		...
		for (auto& reloc : relocTypes)
			LogWarn(
			    "Unsupported Mach-O relocation %s", GetRelocationString((MachoArmRelocationType)reloc));
		return false;
	};
};
