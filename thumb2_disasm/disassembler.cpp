...
int thumb_decompose(struct decomp_request* info, struct decomp_result* result)
{
	...
	else if (result->formatCount == 3 && IS_FIELD_PRESENT(result, FIELD_index) &&
	...
	/* determine the address mode for 2 and 4-format instructions */
	         ...
	         IS_FIELD_PRESENT(result, FIELD_P) && IS_FIELD_PRESENT(result, FIELD_W) &&
	         IS_FIELD_PRESENT(result, FIELD_U))
	{
		...
		if (result->formatCount == 4)
		{
			if (P && !W)
				...
				result->addrMode = ADDRMODE_UNINDEXED;
			else
				result->flags |= FLAG_ADDRMODE_AMBIGUITY;
		}
	...
	if (getenv("DEBUG_DISASM"))
	{
		...
		if (result->status & STATUS_OK)
			printf("  OK\n");
		if (result->status & STATUS_NO_BIT_MATCH)
			printf("  NO_BIT_MATCH\n");
		if (result->status & STATUS_ARCH_UNSUPPORTED)
			printf("  ARCH_UNSUPPORTED\n");
		if (result->status & STATUS_UNDEFINED)
			printf("  UNDEFINED\n");

		printf("decomp_result flags:\n");
		if (result->flags & FLAG_UNPREDICTABLE)
			printf("  UNPREDICTABLE\n");
		if (result->flags & FLAG_NOTPERMITTED)
			printf("  NOTPERMITTED\n");

		printf("address mode: %d\n", result->addrMode);
		if (result->addrMode == ADDRMODE_OFFSET)
			printf("  OFFSET\n");
		if (result->addrMode == ADDRMODE_PREINDEX)
			printf("  PREINDEX\n");
		if (result->addrMode == ADDRMODE_POSTINDEX)
			printf("  POSTINDEX\n");
		if (result->addrMode == ADDRMODE_UNSPECIFIED)
			printf("  UNSPECIFIED\n");
	}
	...
	if (format->operationFlags & INSTR_FORMAT_FLAG_VFP_DATA_SIZE)
	{
		if (IS_FIELD_PRESENT(result, FIELD_dt))
		{
			...
			else if (IS_FIELD_PRESENT(result, FIELD_unsigned))
			{
				switch (result->fields[FIELD_dt])
				{
				case VFP_DATA_SIZE_S8:
					contents += ".S8";
					break;
				case VFP_DATA_SIZE_S16:
					contents += ".S16";
					break;
				case VFP_DATA_SIZE_S32:
					contents += ".S32";
					break;
				case VFP_DATA_SIZE_S64:
					contents += ".S64";
					break;
				case VFP_DATA_SIZE_U8:
					contents += ".U8";
					break;
				case VFP_DATA_SIZE_U16:
					contents += ".U16";
					break;
				case VFP_DATA_SIZE_U32:
					contents += ".U32";
					break;
				case VFP_DATA_SIZE_U64:
					contents += ".U64";
					break;
				case VFP_DATA_SIZE_32:
					contents += ".32";
					break;
				default:
					contents += ".error";
			...
			else if (IS_FIELD_PRESENT(result, FIELD_cmode))
			{
				...
				if (cmode >> 1 <= 3)
					...
					contents += ".I32";
				else if (op == 0 && cmode == 14)
					contents += ".I8";
				else if (op == 0 && cmode == 15)
					contents += ".F32";
				else if (op == 1 && cmode == 14)
					contents += ".I64";
				else if (op == 1 && cmode == 15)
					contents += ".undefined";
				else
					contents += ".error";
			}
			else if (IS_FIELD_PRESENT(result, FIELD_iword))
			{
				switch (result->fields[FIELD_dt])
				{
				case VFP_DATA_SIZE_I8:
					contents += ".I8";
					break;
				case VFP_DATA_SIZE_I16:
					contents += ".I16";
					break;
				case VFP_DATA_SIZE_I32:
					contents += ".I32";
					break;
				case VFP_DATA_SIZE_I64:
					contents += ".I64";
					break;
				case VFP_DATA_SIZE_I_F32:
					contents += ".F32";
					break;
				default:
					contents += ".error";
