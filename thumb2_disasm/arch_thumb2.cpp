...
class Thumb2Architecture : public ArmCommonArchitecture
{
 protected:
	...
	void populateDecomposeRequest(decomp_request* req, const uint8_t* data, size_t len, uint64_t addr,
	    int inIfThen, int inIfThenLast)
	{
		req->instr_word16 = 0;
		req->instr_word32 = 0;
 ...
 public:
	...
	virtual size_t GetInstructionAlignment() const override { return 2; }

	virtual size_t GetOpcodeDisplayLength() const override { return 4; }
	    ...
	    const uint8_t* data, uint64_t addr, size_t maxLen, InstructionInfo& result) override
	{
		...
		case armv7::ARMV7_LDR:
			if ((decomp.format->operands[0].type == OPERAND_FORMAT_REG) &&
			    (decomp.fields[decomp.format->operands[0].field0] == 15))
			{
		...
		case armv7::ARMV7_B:
			    ...
			    (decomp.fields[FIELD_cond] == COND_AL))
			{
				result.AddBranch(UnconditionalBranch,
				    (decomp.fields[decomp.format->operands[0].field0] + 4 + addr) & 0xffffffffLL, this);
			}
			else
			{
				result.AddBranch(TrueBranch,
				    (decomp.fields[decomp.format->operands[0].field0] + 4 + addr) & 0xffffffffLL, this);
		...
		case armv7::ARMV7_CBZ:
			result.AddBranch(TrueBranch,
			    (decomp.fields[decomp.format->operands[1].field0] + addr) & 0xffffffffLL, this);
			result.AddBranch(FalseBranch, (addr + result.length) & 0xffffffffLL, this);
			break;
	...
	*/
	    ...
	    vector<InstructionTextToken>& result) override
	{
		...
		for (size_t i = 0; i < decomp.format->operandCount; i++)
		{
			...
			case OPERAND_FORMAT_REGISTERS: /* register list */
				...
				if (decomp.group == INSN_GROUP_NEON)
				{
					...
					else
					{
						...
						if (IS_FIELD_PRESENT(&decomp, FIELD_d2))
							d2 = decomp.fields[FIELD_d2];
						if (IS_FIELD_PRESENT(&decomp, FIELD_d3))
							d3 = decomp.fields[FIELD_d3];
						if (IS_FIELD_PRESENT(&decomp, FIELD_d4))
			...
			{
				const char* lookup[16] = {"#0x0", "#0x1", "OSHST", "OSH", "#0x4", "#0x5", "NSHST", "NSH",
				    "#0x8", "#0x9", "ISHST", "ISH", "#0xC", "#0xD", "ST", "SY"};
			...
			case OPERAND_FORMAT_LABEL: /* <label> field becomes [PC,#<+/-><imm32>] */
				...
				{
					if (imm32 < 10)
						sprintf(offset, "%d", imm32);
					else
						sprintf(offset, "0x%x", imm32);
	    ...
	    const uint8_t* data, uint64_t addr, size_t& len, LowLevelILFunction& il) override
	{
		...
		if ((decomp.mnem == armv7::ARMV7_IT) && (decomp.fields[FIELD_mask] != 0))
		{
			...
			{
				populateDecomposeRequest(&request, data + offset, len - offset, addr + offset, IFTHEN_YES,
				    ((i + 1) >= instrCount) ? IFTHENLAST_YES : IFTHENLAST_NO);
